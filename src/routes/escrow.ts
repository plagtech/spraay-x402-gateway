...
interface Escrow {
  id: string; depositor: string; beneficiary: string; arbiter: string | null;
  token: EscrowToken; amount: string; amountRaw: string; description: string | null;
  conditions: string[]; status: EscrowStatus; expiresAt: string | null;
  fundedAt: string | null; releasedAt: string | null; cancelledAt: string | null;
  releaseTxHash: string | null; createdAt: string; updatedAt: string;
}
...
function lookupEscrow(id: string): Escrow | null {
  if (!id) return null;
  const escrow = escrowStore.get(id.toUpperCase());
...
export async function escrowFundHandler(req: Request, res: Response) {
  try {
    ...
    const escrow = lookupEscrow(escrowId);
    ...
    return res.json({
      status: "funded",
      escrow: { id: escrow.id, status: "funded", amount: escrow.amount, token: escrow.token.symbol, fundedAt: escrow.fundedAt, beneficiary: escrow.beneficiary },
...
export async function escrowReleaseHandler(req: Request, res: Response) {
  try {
    ...
    return res.json({
      status: "released",
      escrow: { id: escrow.id, status: "released", amount: escrow.amount, token: escrow.token.symbol, depositor: escrow.depositor, beneficiary: escrow.beneficiary, releasedBy: isDepositor ? "depositor" : "arbiter", releasedAt: escrow.releasedAt },
...
export async function escrowCancelHandler(req: Request, res: Response) {
  try {
    ...
    return res.json({
      status: "cancelled",
      escrow: { id: escrow.id, status: "cancelled", amount: escrow.amount, token: escrow.token.symbol, cancelledBy: isDepositor ? "depositor" : "arbiter", cancelledAt: escrow.cancelledAt, wasFunded: escrow.fundedAt !== null },
      _gateway: { provider: "spraay-x402", version: "2.8.0" },
      timestamp: new Date().toISOString(),
    });
  } catch (error: any) {
...
export async function escrowListHandler(req: Request, res: Response) {
  try {
    ...
    for (const escrow of escrowStore.values()) {
      ...
      results.push({ id: escrow.id, role: isDepositor ? "depositor" : isBeneficiary ? "beneficiary" : "arbiter", depositor: escrow.depositor, beneficiary: escrow.beneficiary, token: escrow.token.symbol, amount: escrow.amount, status: escrow.status, description: escrow.description, expiresAt: escrow.expiresAt, createdAt: escrow.createdAt });
    }

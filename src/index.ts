...
const paidRoutes = {
      ...
      "POST /api/v1/batch/estimate": {
        accepts: [{ scheme: "exact", price: "$0.001", network: CAIP2_NETWORK, payTo: PAY_TO }, { scheme: "exact", price: "$0.001", network: SOLANA_NETWORK, payTo: SOLANA_PAY_TO }],
        description: "Estimate batch gas and protocol fee. Units: amounts[] are RAW base units (USDC 6dp: \"1500000\" = $1.50); totalAmount/fee/totalWithFee are human-decimal strings; suggestedGasLimit is a decimal string.", mimeType: "application/json",
        extensions: { ...declareDiscoveryExtension({ input: { token: "USDC", recipients: ["0x1111111111111111111111111111111111111111", "0x2222222222222222222222222222222222222222"], amounts: ["1500000", "1500000"] }, inputSchema: { properties: { token: { type: "string" }, recipients: { type: "array" }, amounts: { type: "array" }, sender: { type: "string" }, recipientCount: { type: "number" } }, required: [] }, bodyType: "json", output: { example: { success: true, token: { symbol: "USDC", address: "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913", decimals: 6, isETH: false }, recipientCount: 2, totalAmount: "3.0", fee: "0.009", feePercent: "0.3%", totalWithFee: "3.009", suggestedGasLimit: "230000" }, schema: { properties: { success: { type: "boolean" }, token: { type: "object" }, recipientCount: { type: "number" }, totalAmount: { type: "string" }, fee: { type: "string" }, feePercent: { type: "string" }, totalWithFee: { type: "string" }, suggestedGasLimit: { type: "string" }, estimatedGas: { type: "string" }, note: { type: "string" } } } } }) },
      },
...
const MANIFEST_META = [
      ...
      { resource: `${BASE_URL}/free/estimate-batch`, method: "GET",  price: "free", category: "free-tier", description: "Rough cost estimate for batch payments by recipient count and chain. Returns estimate.supported — whether a batch can actually settle on that chain — alongside estimate.supportedChains. No payment required.", searchTerms: ["batch estimate","rough estimate","cost preview","free estimate"] },
  ...
  const endpoints = [
    ...
    { method: "post", path: "/api/v1/batch/execute", price: "$0.02", priceNum: "0.020000", tag: "payments", desc: "Batch USDC payments on Base or peaq (amounts[] in RAW base units)",
      ...
      outputProps: {
        ...
        contract: { type: "string", description: "Spraay contract the transaction targets." },
        token: { type: "object", properties: { symbol: { type: "string" }, address: { type: "string" }, decimals: { type: "number" }, isETH: { type: "boolean" } } },
        batch: { type: "object", description: "Human-readable summary. Every amount here is a HUMAN-DECIMAL string, not base units.", properties: { recipientCount: { type: "number" }, totalAmount: { type: "string", description: "Human decimal, e.g. \"1.5\"." }, fee: { type: "string", description: "Human decimal. 0.3% protocol fee." }, feePercent: { type: "string" }, totalWithFee: { type: "string", description: "Human decimal. totalAmount + fee." } } },
        transaction: { type: "object", description: "Unsigned transaction to sign and broadcast.", properties: { to: { type: "string" }, data: { type: "string", description: "Hex calldata." }, value: { type: "string", description: "RAW wei as a decimal string. \"0\" for ERC-20 batches." }, chainId: { type: "number" }, gasLimit: { type: "string", description: "HEX quantity, e.g. \"0x38270\"." } } },
        approvalRequired: { type: "object", description: "ERC-20 batches only; absent for native ETH. Approve before broadcasting the transaction.", properties: { token: { type: "string" }, spender: { type: "string" }, amount: { type: "string", description: "RAW base units. Pass this value verbatim as the approve() allowance." }, amountFormatted: { type: "string", description: "The same allowance as a human decimal, for display only." } } },
      } },
    { method: "post", path: "/api/v1/batch/estimate", price: "$0.001", priceNum: "0.001000", tag: "payments", desc: "Estimate batch gas and fee (amounts[] in RAW base units)",
      inputProps: {
        token: { type: "string", description: "Same token input as batch/execute. Defaults to USDC." },
        recipients: { type: "array", description: "Detailed form: recipient addresses, parallel to amounts[]. Omit for the count-only form." },
      ...
      outputProps: {
        ...
        totalAmount: { type: "string", description: "Detailed form only. HUMAN-DECIMAL string." },
        fee: { type: "string", description: "Detailed form only. HUMAN-DECIMAL string. 0.3% protocol fee." },
        feePercent: { type: "string" },
        totalWithFee: { type: "string", description: "Detailed form only. HUMAN-DECIMAL string." },
        suggestedGasLimit: { type: "string", description: "DECIMAL string — note this differs from batch/execute's transaction.gasLimit, which is hex." },
        estimatedGas: { type: "string", description: "Count-only form. DECIMAL string, cold-recipient worst case." },
        note: { type: "string", description: "Count-only form. Explains that the figure is a conservative ceiling." },
      } },
